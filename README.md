...
Create an `application.properties` file (or set the equivalent environment variables) with the following configuration:
...
> **Note:** Never commit real credentials or secrets to version control. Use a `.gitignore` entry for `application.properties` or load these values from environment variables in production.
...
Open `index.html` directly in a browser, or serve it with a local static server (e.g. the VS Code "Live Server" extension) so relative API calls resolve correctly.
