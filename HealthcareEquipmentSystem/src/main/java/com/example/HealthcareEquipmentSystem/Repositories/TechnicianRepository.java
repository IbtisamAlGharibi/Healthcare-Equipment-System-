...
public interface TechnicianRepository extends JpaRepository<MaintenanceTechnician, Integer> {
    @Query("SELECT S FROM MaintenanceTechnicia M WHERE M.isActive=true")
    ...
    @Query("SELECT S FROM MaintenanceTechnicia M WHERE M.specialization= :specialization AND M.isActive=true")
